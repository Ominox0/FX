...
struct ImportDecl { string name, version, alias; };
...
struct Parser {
    ...
    ImportDecl parseImport(){ expect(TokKind::IMPORT); string name=expect(TokKind::IDENT).text; expect(TokKind::COLON); string ver; while(peek().kind==TokKind::INT_LITERAL || peek().kind==TokKind::FLOAT_LITERAL || peek().kind==TokKind::DOT){ if(peek().kind==TokKind::INT_LITERAL){ ver+=expect(TokKind::INT_LITERAL).text; continue; } if(peek().kind==TokKind::FLOAT_LITERAL){ ver+=expect(TokKind::FLOAT_LITERAL).text; continue; } if(peek().kind==TokKind::DOT){ ver+="."; expect(TokKind::DOT); continue; } break; } expect(TokKind::AS); string alias=expect(TokKind::IDENT).text; return ImportDecl{name,ver,alias}; }
...
struct Interpreter{
    ...
    void exec_stmt(const StmtPtr& st, shared_ptr<Env> env){ if(auto v = dynamic_pointer_cast<VarDeclStmt>(st)){ Value val=eval_expr(v->value, env); env->define(v->name, val); return; } if(auto e = dynamic_pointer_cast<ExprStmt>(st)){ eval_expr(e->expr, env); return; } if(auto w = dynamic_pointer_cast<WhileStmt>(st)){ while(truthy(eval_expr(w->cond, env))){ try{ exec_block(w->body, make_shared<Env>(env)); } catch(BreakSignal&){ break; } } return; } if(auto f = dynamic_pointer_cast<ForStmt>(st)){ auto local=make_shared<Env>(env); if(f->init) eval_expr(f->init, local); while(true){ if(f->cond && !truthy(eval_expr(f->cond, local))) break; try{ exec_block(f->body, make_shared<Env>(local)); } catch(BreakSignal&){ break; } if(f->post){ if(dynamic_pointer_cast<IdentExpr>(f->post)){ auto id=static_pointer_cast<IdentExpr>(f->post)->name; Value cur=local->get(id); long long nv=num_to_int(cur)+1; local->set(id, make_int(nv)); } else eval_expr(f->post, local); } } return; } if(auto i = dynamic_pointer_cast<IfStmt>(st)){ if(truthy(eval_expr(i->cond, env))){ exec_block(i->thenBody, make_shared<Env>(env)); return; } for(auto &el: i->elifs){ if(truthy(eval_expr(el.first, env))){ exec_block(el.second, make_shared<Env>(env)); return; } } if(!i->elseBody.empty()) exec_block(i->elseBody, make_shared<Env>(env)); return; } if(dynamic_pointer_cast<BreakStmt>(st)){ throw BreakSignal{}; } if(auto r = dynamic_pointer_cast<ReturnStmt>(st)){ Value v; if(r->value) v=eval_expr(r->value, env); throw ReturnSignal{v}; } throw runtime_error("Unknown statement"); }
